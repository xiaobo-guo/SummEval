...
class SupertMetric(Metric):
    ...
    def evaluate_batch(self, summaries, input_texts, aggregate=True, show_progress_bar=False):
        ...
        results = []
        for summ, input_text in tqdm.tqdm(zip(summaries, input_texts),total=len(summaries),disable=not show_progress_bar, dynamic_ncols=True, desc='Calculate Supert',leave=False):
